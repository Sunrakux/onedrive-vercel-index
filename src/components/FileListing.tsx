...
// Dynamic import for browser-only previews
...
export const Checkbox: FC<{
  ...
  const handleClick: MouseEventHandler = e => {
    if (ref.current) {
      if (e.target === ref.current) e.stopPropagation()
  ...
  return (
    <span
      ...
      onClick={handleClick}
    >
      <input className="form-check-input cursor-pointer" type="checkbox" value={checked ? '1' : ''} ref={ref} aria-label={title} onChange={onChange} />
    </span>
  )
}
...
const FileListing: FC<{ query?: ParsedUrlQuery }> = ({ query }) => {
  ...
  if (!data) return <Loading loadingText={t('Loading ...')} />
  ...
  if ('folder' in responses[0]) {
    ...
    const toggleTotalSelected = () => {
      if (genTotalSelected(selected) == 2) {
        setSelected({})
        setTotalSelected(0)
      } else setSelected(Object.fromEntries(getFiles().map(c => [c.id, true]))), setTotalSelected(2)
    }

    // Download and folder functions omitted for brevity (same as original)
    // ...

    const folderProps = { toast, path, folderChildren, selected, toggleItemSelected, totalSelected, toggleTotalSelected, totalGenerating, folderGenerating }
    return (
      <>
        <Toaster />
  ...
  if ('file' in responses[0] && responses.length === 1) {
    ...
    } else return <DefaultPreview file={file} />
  }

  return <FourOhFour errorMsg={t('Cannot preview {{path}}', { path })} />
}
